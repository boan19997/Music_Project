...
//lấy tất cả bài hát trong một albums
router.post("/getallsonginalbum", async (req, res) => {
  const { namealbum } = req.body;

  if(!namealbum){
    return res.status(400).send({ success: false, msg: "not name album" });
  }

  try {
    const data = await song.find({ album: namealbum });
